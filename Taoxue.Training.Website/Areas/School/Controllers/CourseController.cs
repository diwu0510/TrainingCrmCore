using System;
using System.Collections.Generic;
using System.Linq;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using HZC.Core;
using Taoxue.Training.Services;
using Taoxue.Training.Website.Extensions;

namespace Taoxue.Training.WebSite.Areas.School.Controllers
{
    //[Authorize]
    public class CourseController : SchoolBaseController
    {
        private readonly CourseService service = new CourseService();

        public CourseController()
        { }

        #region 列表页
        public ActionResult Index()
        {
            InitUI();
            return View();
        }

        public JsonResult Get(CourseSearchParam param)
        {
            var list = service.Fetch(param);
            return Json(ResultUtil.Success<IEnumerable<CourseEntity>>(list));
        }
        #endregion

        #region 创建
        public IActionResult Create()
        {
            var entity = new CourseEntity();
            entity.UnitPrice = 1;
            entity.Enabled = true;
            return View(entity);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Create(IFormCollection collection)
        {
            CourseEntity entity = new CourseEntity();
            TryUpdateModelAsync(entity);

            entity.SchoolId = SchoolUser.SchoolId;

            var result = service.Create(entity, SchoolUser);
            // 如果有缓存，注意在这里要清空缓存

            return Json(result);
        }
        #endregion

        #region 修改
        public IActionResult Edit(int id)
        {
            var entity = service.Load(id);
            if (entity == null)
            {
                return NotFound();
            }
            else
            {
                InitUI();
                return View(entity);
            }
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Edit(int id, IFormCollection collection)
        {
            CourseEntity entity = new CourseEntity();
            TryUpdateModelAsync(entity);
            var result = service.Update(entity, SchoolUser);
            // 如果有缓存，注意在这里要清空缓存

            return Json(result);
        }
        #endregion

        #region 删除
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Delete(int id)
        {
            var entity = service.Load(id);
            if (entity == null)
            {
                return Json(ResultUtil.AuthFail("请求的数据不存在"));
            }
            var result =  service.Remove(entity, SchoolUser);
            // 如果有缓存，注意在这里要清空缓存

            return Json(result);
        }
        #endregion

        #region 辅助方法
        private void InitUI()
        {
        }
        #endregion
    }
}
